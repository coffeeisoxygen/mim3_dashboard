"""SessionRepository ORM implementation menggunakan SQLAlchemy models."""
...
class SessionRepositoryORM:
    """Concrete implementation untuk SessionRepository menggunakan ORM models.

    Implements SessionRepository protocol dengan SQLAlchemy ORM untuk:
    - Auto datetime conversion
    - Type safety dengan relationships
        ...
        try:
            ...
            with conn.session as s:
                ...
                stmt = (
                    ...
                    .options(
                        selectinload(UserSessionORM.user).selectinload(
                            UserAccountORM.role
                        )
                    )
                ...
                if session_obj.is_expired():  # ✅ Use ORM method
            ...
            with conn.session as s:
                ...
                if DatabaseHelper.safe_commit(s):
                    ...
                    return SessionResult.error_result(
                        "Gagal commit session ke database"
                    )
            ...
            with conn.session as s:
                ...
                stmt = (
                    ...
                    .where(
                        ...
                        & (UserSessionORM.is_active == True)  # noqa: E712
                    )
                ...
                if success:
                    logger.debug(
                        f"Session activity updated (ORM): {session_token[:10]}..."
                    )
                else:
                    logger.warning(
        ...
        try:
            ...
            with conn.session as s:
                ...
                stmt = (
                    ...
                    .where(UserSessionORM.is_active == True)  # noqa: E712
                )
                ...
                # ✅ Convert ORM objects to dict untuk protocol compliance
                session_data = [
                    {
                        "id": session.id,
                ...
                logger.debug(f"Found {len(session_data)} active sessions")
                return session_data
    ...
    def create_session_factory(
        self, user_id: int, hours: int = 8, request_info: dict | None = None
    ) -> SessionResult:
...
# PINNED: Factory method untuk easy instantiation
def get_session_repository_orm() -> SessionRepository:
    """Factory untuk create SessionRepository ORM implementation."""
    return SessionRepositoryORM()


# TODO: Update SessionService untuk use repository pattern
# REMINDER: Hapus session_commands.py setelah migration complete
# [ ] PINNED: Test ORM implementation sebelum remove legacy commands
